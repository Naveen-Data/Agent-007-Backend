...
async def receive_frontend_logs(request: Request, log_request: FrontendLogsRequest):
    ...
    for log_entry in log_request.logs:
        ...
        logger.log(log_level, f"[Frontend] {log_entry.message}", extra=extra_data)
...
async def receive_performance_metrics(
    request: Request, metrics_request: PerformanceMetricsRequest
):
    ...
    for metric in metrics_request.metrics:
        ...
        # Log slow operations
        if (
            metric.name in ['page_load', 'api_call'] and metric.value > 3000
        ):  # 3 seconds
            logger.warning(
