...
class ChatRequest(BaseModel):
    ...
    mode: str | None = AgentConstants.MODE_CHAT  # "chat" (default), "rag", "tools", "expressive", or "enhanced_tools"
...
class RAGResponse(BaseModel):
    ...
    sources_used: List[str] = Field(default_factory=list, description="List of document sources referenced")
    context_relevance: float = Field(default=1.0, description="How relevant the retrieved context was (0.0 to 1.0)")
...
class ToolExecutionResult(BaseModel):
    ...
    parameters_used: Dict[str, Any] = Field(default_factory=dict, description="Parameters that were passed to the tool")
...
class GeneralResponse(BaseModel):
    ...
    topics: List[str] = Field(default_factory=list, description="Main topics covered in the response")
...
class ConversationAnalysis(BaseModel):
    ...
    context_summary: str = Field(..., description="Brief summary of conversation context")
    relevant_history: List[str] = Field(default_factory=list, description="Key points from conversation history")
...
class ConversationSummary(BaseModel):
    ...
    title: str = Field(..., description="Concise title for the conversation (3-5 words)")
    ...
    conversation_type: Literal["question_answer", "problem_solving", "information_request", "casual_chat", "technical_discussion"] = Field(
