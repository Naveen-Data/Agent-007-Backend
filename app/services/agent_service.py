...
Select the most appropriate tool and extract the necessary parameters. If no specific tool is needed for a general knowledge question, use "llm_only" as the tool name."""
        ...
        try:
            ...
            tool_decision = await self.llm.generate_structured(
                tool_selection_prompt, ToolSelectionResponse
            )

            if tool_decision is None:
                logger.warning(
                    "Structured generation failed, falling back to context-aware LLM response"
                )
...
{result}"""
    ...
    def _extract_location(self, text: str) -> str:
        ...
        for pattern in patterns:
            ...
            if match:
                ...
                location = re.sub(
                    r"\b(the|city|of|weather|temperature|now)\b",
                    "",
                    location,
                    flags=re.IGNORECASE,
                )
        ...
        for i, word in enumerate(words):
            if word.lower() in ["weather", "temperature"] and i + 1 < len(words):
                ...
                potential_location = re.sub(
                    r"[^A-Za-z\s,]", "", potential_location
                ).strip()
    ...
    async def _llm_fallback_with_context(
        self, question: str, available_tools: dict
    ) -> str:
