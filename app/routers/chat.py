...
async def send(
    ...
    if not req.message:
        logger.warning("Empty message received", extra={'request_id': getattr(request.state, 'request_id', None)})
        raise HTTPException(status_code=400, detail="message required")

    start_time = time.time()
    request_id = req.request_id or getattr(request.state, 'request_id', f'unknown_{int(time.time())}')
    ...
    try:
        ...
        agent = AgentService(
            mode=(req.mode or "rag"), llm=llm, tool_service=tool_service, vectorstore=vectorstore
        )
        
        # Pass conversation history directly to the agent for better memory handling
        reply = await agent.answer_with_history(req.message, req.conversation_history or [])
