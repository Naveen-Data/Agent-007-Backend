...
class LoggingConstants:
    ...
    DETAILED_LOG_FORMAT: Final[str] = "%(asctime)s | %(levelname)-8s | %(name)-20s | %(funcName)-20s:%(lineno)-4d | %(message)s"
...
class AgentConstants:
    ...
    # Default Prompts
    DEFAULT_SYSTEM_PROMPT: Final[str] = """<system>
You are Agent 007, a helpful AI assistant.
</system>"""
    RAG_SYSTEM_PROMPT: Final[str] = """<system>
...
acknowledge this and provide the best answer you can with available information.
    ...
    TOOL_SYSTEM_PROMPT: Final[str] = """<system>
...
Use tools when they can provide more current, specific, or detailed information than your training data.
    ...
    ENHANCED_TOOL_SYSTEM_PROMPT: Final[str] = """<system>
...
def get_all_constants() -> Dict[str, Dict[str, any]]:
    ...
    return {
        ...
        "logging": {k: v for k, v in LoggingConstants.__dict__.items() if not k.startswith('_')},
        "agent": {k: v for k, v in AgentConstants.__dict__.items() if not k.startswith('_')},
        "tools": {k: v for k, v in ToolConstants.__dict__.items() if not k.startswith('_')},
        "database": {k: v for k, v in DatabaseConstants.__dict__.items() if not k.startswith('_')},
        "security": {k: v for k, v in SecurityConstants.__dict__.items() if not k.startswith('_')},
        "errors": {k: v for k, v in ErrorConstants.__dict__.items() if not k.startswith('_')},
        "performance": {k: v for k, v in PerformanceConstants.__dict__.items() if not k.startswith('_')},
        "features": {k: v for k, v in FeatureFlags.__dict__.items() if not k.startswith('_')},
        "version": {k: v for k, v in VersionConstants.__dict__.items() if not k.startswith('_')},
    }
